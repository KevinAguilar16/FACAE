...
class Maestrante_model extends CI_model {
	...
	function lista_estados($idmte){
 	...
 	function maestrante( $id){
 		$maestrante = $this->db->query('select * from maestrante where idmaestrante="'. $id.'"');
 		return $maestrante;
 	}
  ...
  // Para ir al primero
	...
	function elprimero()
	{
		$query=$this->db->order_by("idmaestrante")->get('maestrante');
		if($query->num_rows()>0)
		{
...
// Para ir al último registro
	...
	// Para moverse al siguiente registro
 	function siguiente($id){
 		$maestrante = $this->db->select("idmaestrante")->order_by("idmaestrante")->get('maestrante')->result_array();
		$arr=array("idmaestrante"=>$id);
		$clave=array_search($arr,$maestrante);
	   if(array_key_exists($clave+1,$maestrante))
		 {

 		$maestrante = $this->db->query('select * from maestrante where idmaestrante="'. $maestrante[$clave+1]["idmaestrante"].'"');
		 }else{
